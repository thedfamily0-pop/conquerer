...
interface QuestMapProps { onEarnXp: (amount: number, activityKey?: string) => void; soundEnabled: boolean; displayName: string; }
...
export function QuestMap({ onEarnXp, soundEnabled, displayName: _displayName }: QuestMapProps) {
  ...
  const handleSubmit = () => {
    ...
    if (evaluation.score !== 'retry') {
      ...
      if (awardedStars > 0) onEarnXp(awardedStars * 10, `quest:${nodeId}:checkpoint:${checkpointIdx}`);
  ...
  return (<section className="glass-card animate-pop" style={{ padding: '24px' }}>
    ...
    <div style={{ background: 'rgba(15, 23, 42, 0.5)', border: `1px solid ${levelColors[cp.level]}33`, borderRadius: '16px', padding: '18px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}><span style={{ fontSize: '0.75rem', color: answer.length < 15 ? '#f87171' : '#64748b' }}>{answer.length} chars {answer.length < 15 ? '— one more idea may help!' : ''}</span><span style={{ fontSize: '0.75rem', color: '#64748b' }}>{cp.stars} star{cp.stars > 1 ? 's' : ''} possible</span></div>
