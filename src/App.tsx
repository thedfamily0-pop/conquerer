...
export function App() {
  ...
  useEffect(() => {
    ...
    const syncEveryFifteenMinutes = async () => {
      ...
      try {
        if (accessRole === 'child') {
          await syncActiveSession({ diary: diaryRef.current, nomiMessages: nomiMessagesRef.current, performanceEvents: getPerformanceEvents(), vocab: loadVocab() });
        }
