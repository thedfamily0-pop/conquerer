...
| Interactive Vocab Book — automatic English meanings, editable examples, term/week stamps, Wrapped stats | Secure Gemini gateway for parent AI (development-only BYOK fallback) |
...
- **Safe active-session sync**: while a hosted session is open, visible, and online, it retries every 15 minutes and on reconnect/focus. Child-originated diary, Nomi, learning evidence, and saved vocabulary use upsert-only synchronization before the device refreshes remote state. Schedules, chores, wallet, and store snapshots are deliberately excluded from this timer because a stale device could overwrite or delete newer records. Closed/background browser sessions cannot be guaranteed to run a web timer.
- **Child profile customisation**: migration `021_safe_child_profile_customization.sql` adds an allowlisted child-only RPC for display name, avatar, skin, background, and AI companion name. It never changes profile ownership, family membership, role, photos, or voice preference; photos and voice remain device-local.
- **Interactive Vocab Book**: English words are looked up on demand after a short pause or with **Find meaning**. The response can be edited before saving, and a 200-entry, 90-day device cache supports repeat lookups. The cache is not a shared dictionary dataset; only the child’s saved vocabulary rows sync to `vocab_words`.
- **Oxford boundary**: no Oxford content is seeded or cached. An Oxford production integration requires a suitable licence and a server-side provider integration for its credentials; see the [Oxford API terms](https://developer.oxforddictionaries.com/api-terms-and-conditions).
...
- **Child AI allowance alerts**: migration `019_child_ai_quota_alerts_and_daily_overrides.sql` keeps hosted child usage server-authoritative and sends at most one parent alert per Johannesburg day when a child reaches 95% of the applicable enforced total, Nomi, or homework cap. Parent Zone can only raise a linked child’s total/Nomi/homework caps for that Johannesburg day; the server rejects decreases, direct browser table access, and ambiguous multi-child targeting. The one-child day-only UI intentionally fails closed when more than one linked child account exists until child selection is designed.
- **Answer evidence**: when hosted sync is enabled, a learner’s final submitted practice response is retained in `learning_performance_events.answer` alongside correctness, score, hints, and timestamp. It remains outside daily aggregate snapshots. The weekly research job may inspect bounded English/Afrikaans response evidence to identify language patterns, but its prompt prohibits quoting those responses or passing them to the parent research-LLM handover.
...
- **Per-profile PINs and recovery**: migrations `010_access_roles_and_portal_pins.sql`, `013_family_access_fail_closed.sql`, and `018_google_reauth_portal_pin_recovery.sql` use bcrypt-backed credentials, lockout-aware verification, and authenticated-user-ID family approval. A parent who forgets the Parent Zone PIN begins a 15-minute server-stored, hash-only challenge, completes a fresh Google login with `prompt=login`, then chooses a new 4–12 digit PIN. The raw challenge exists only in browser `sessionStorage` during that OAuth return—never in a URL, localStorage, database, log, or email. A child asks for a personal-PIN reset; a matching-family parent must freshly re-authenticate with Google before choosing and approving the replacement PIN. On a first hosted Parent Zone visit, a valid 4–12 digit PIN is enrolled for the authenticated parent. Hosted mode fails closed when verification is unavailable. Offline-only mode retains a device-local PIN compatibility fallback and is not a shared-device security boundary.

### Hosted access, invitations, and contact settings

- **Migrations**: `001`–`006` establish the original schema; `007_production_hardening.sql` adds per-channel quota columns, family-scoped replacement policies, UUID-safe Nomi IDs, and the atomic quota function. `008_learning_events_retention.sql` supports hosted learning events and one-day alert pruning. `011_learning_results_reports_xp.sql` adds historical results, SMART goals, report settings, and atomic learning XP claims. `012_security_followups.sql` hardens learning-event child/family consistency, parent report visibility, and parent-only wallet writes. `013_family_access_fail_closed.sql` prevents contact email arrays from granting access. `014_family_invitations_google_onboarding.sql` adds server-managed invitation records, family administrators, Google-only acceptance, and one-time family membership binding. `015_fix_invitation_pgcrypto_schema.sql` qualifies restricted-schema token helpers. `016_practice_question_teaching_videos.sql` adds reviewed lesson metadata and varied activity fields. `017_learning_report_snapshots_and_research_drafts.sql` adds aggregate daily snapshots and parent-only weekly research drafts. `018_google_reauth_portal_pin_recovery.sql` adds hash-only Google re-auth challenges and parent-approved child-PIN reset requests. `019_child_ai_quota_alerts_and_daily_overrides.sql` adds child day-only allowance overrides and retryable 95%-threshold alert claims. `020_seed_remaining_2026_learning_calendar_weeks.sql` adds the reviewed remaining 2026 Term 3/4 calendar rows, stopping at the declared term end dates. `021_safe_child_profile_customization.sql` adds the authenticated child-only profile-customisation RPC without changing existing profile rows.
...
### Current production release and first-family procedure

The production project (`gbjkockgfntgctchkzdk`) has the application schema through migration 020, Google-only onboarding, and the reviewed active `ai-chat`, `send-parent-alert`, `send-parent-reports`, `materialize-learning-report-snapshots`, `generate-weekly-content-research`, and `send-family-invitation` Edge Functions. GitHub Pages deployed commit `3cea5f5` successfully, and `https://thedfamily0-pop.github.io/conquerer/` returned HTTP 200. The invitation Function retains its production browser origin (`https://thedfamily0-pop.github.io`) and full GitHub Pages invitation URL (`https://thedfamily0-pop.github.io/conquerer/`) server-side, never as Vite variables.

Five active production `pg_cron` jobs use `pg_net` with tokens resolved only from Vault: daily and Saturday parent reports, daily learning snapshots, Saturday weekly-content research, and 15-minute child-AI quota-alert retry. The four scheduler tokens were freshly rotated in Function Secrets and Vault without being printed or committed. One controlled no-family invocation of each protected endpoint returned HTTP 200 with zero processed/sent/created/retryable records; no report, alert, research draft, or Gemini family call was produced.

A reviewed dependency-aware production reset removed all user/content rows, including families, memberships, profiles, invitations, contact settings, guardrails, PIN credentials, reports, learning events, AI quota records, and imported content. `auth.users` and `auth.identities` both verified at zero. Storage had zero objects and zero buckets. The schema, migrations, RLS/RPCs, Google provider configuration, Edge Functions/secrets, sender configuration, and all 18 reviewed `learning_calendar_weeks` rows were retained. **A real welcome email and an end-to-end parent or child redemption remain untested; do not treat provider delivery as verified until an authorised recipient receives and redeems an invitation.**

To begin the fresh first-family journey:
...
- Floating 📖 button accessible from anywhere; saved words retain term/week stamps and support search, filtering, and deletion
- English words populate a short meaning and optional example automatically after a brief pause or through **Find meaning**; the learner can always edit either field before saving
- Lookup results use a bounded 200-entry, 90-day local cache. Saved vocabulary—not the lookup cache—syncs to `vocab_words` during safe active-session sync
- Afrikaans, isiZulu, and other languages remain manual in this release; offline or unavailable lookup also leaves the manual meaning field available
- No Oxford dataset is copied into the application or database. A licensed Oxford API integration remains a future server-side provider option
- `getWrappedStats()` remains ready for an end-of-term Spotify-Wrapped-style infographic

### Content pipeline

- **📥 Download Template**: markdown with theme instructions, LLM formatting rules, current-week ATP objectives, varied activity formats, and a parent-reviewed teaching-video requirement
- **📤 Upload Content**: JSON import merges schedule, chores, stories, questions, objectives; new imported practice questions require a valid reviewed YouTube lesson and remain locked until it finishes
- **Template video boundary**: Gemini supplies exact search queries or a one-to-two-minute fallback visual-lesson brief; it never invents source/YouTube URLs, and automatic video production is not implemented
- **Weekly research foundations**: the server-only snapshot/research jobs create parent-reviewable drafts from canonical calendar outcomes and aggregates; they do not publish content or email anyone. The protected production jobs are deployed, Vault-backed, and scheduled; parent review/upload remains required before publication.
...
1. Apply migrations `001` through `020` in numeric order. Migration 014 adds Google-only invitation onboarding; migration 015 fixes its pgcrypto schema qualification; migration 016 retains parent-reviewed teaching-video metadata for hosted practice content; migration 017 adds immutable Johannesburg daily learning snapshots, calendar-owned weekly research drafts, and parent-only draft visibility; migration 018 adds hash-only Google re-auth PIN recovery and parent-approved child PIN-reset requests; migration 019 adds one-day child AI overrides and quota-alert claims; migration 020 supplies the remaining reviewed 2026 Term 3/4 calendar rows. Do not use ordinary migration-history shortcuts for the historical short filenames.
...
3. Deploy `supabase/functions/ai-chat/index.ts` and `supabase/functions/send-parent-alert/index.ts` as a coordinated quota release. Set `GEMINI_API_KEY`, `RESEND_API_KEY`, `RESEND_ALERTS_FROM_EMAIL`, and the same high-entropy `AI_QUOTA_ALERT_INTERNAL_TOKEN` in both Functions; never put any of those values in Vite, Pages variables, browser storage, Git, or documentation. The alert Function accepts only the fixed 95%-threshold claim or its fixed retry request.
4. Set the remaining Function-specific sender secrets: `RESEND_WELCOME_FROM_EMAIL`, `RESEND_DAILY_RECAP_FROM_EMAIL`, and `RESEND_WEEKLY_RECAP_FROM_EMAIL`. Use verified sender addresses with display names, deploy the matching Functions, and verify the sender domain. Before sending real invitations, verify `RESEND_TEST_RECIPIENT` is absent or not constraining a verified-domain sender.
...
6. Deploy `supabase/functions/send-parent-reports/index.ts`. Set `REPORTS_CRON_TOKEN` as a Supabase Function Secret; keep it server-side and never put it in the frontend. Daily reports use `RESEND_DAILY_RECAP_FROM_EMAIL`; weekly reports use `RESEND_WEEKLY_RECAP_FROM_EMAIL`. The function selects only Dad email 1 and Mom email 1.
7. Configure protected hosted invocations, never GitHub Pages jobs: call `send-parent-reports` with `Authorization: Bearer <REPORTS_CRON_TOKEN>` and `{ "kind": "all" }` at **20:30 daily** (`30 18 * * *` UTC) and **13:00 Saturday** (`0 11 * * 6` UTC); call `materialize-learning-report-snapshots` with its separate token at **00:15 Johannesburg daily** (`15 22 * * *` UTC, which materializes the preceding local day); call `generate-weekly-content-research` with its separate token at **12:00 Saturday Johannesburg** (`0 10 * * 6` UTC); and call `send-parent-alert` with `x-conquerer-quota-alert-token` plus `{ "kind": "retry_child_ai_quota_alerts" }` every 15 minutes. All four schedules must use protected server-side secrets. Research runs before the weekly recap and never publishes content or sends email.
8. Run the snapshot/research/alert-retry jobs once in staging with protected credentials and inspect their stored output/status before activating their production schedules. Do not send a real invitation, alert, or report as the smoke test.
9. Set Vite variables in the hosting build environment:
   - `VITE_SUPABASE_SYNC_ENABLED=true`
   - `VITE_AI_GATEWAY_ENABLED=true`
   - `VITE_SUPABASE_URL=...`
   - `VITE_SUPABASE_ANON_KEY=...`
10. Leave `VITE_ALLOW_DIRECT_AI` unset/false in production.
11. Verify the bootstrap Google account, a parent invitation, a child invitation, invitation rejection for an uninvited Google account, parent first-use PIN enrollment, recovery rejection without a fresh/valid matching Google challenge, successful parent PIN recovery after fresh Google login, child self-request, and matching-family parent re-authenticated child-PIN approval. Also verify remote hydration, atomic cross-device learning XP claims, diary/schedule/chore/Nomi/store sync, family isolation, quota responses (30 Nomi / 10 homework / 5 parent AI per day by default), and offline fallback. Test the XP RPC with two authenticated sessions: 70 XP then 50 XP must award exactly 100 total, and replaying the same `p_client_id` must not increase the wallet.
12. Test daily snapshot idempotency, Johannesburg midnight windows, parent-only snapshot/draft access, failed/invalid Gemini research output, and no automatic publication in a non-production environment before enabling either job.
13. Test the invitation and report functions with a non-production recipient configuration before enabling production delivery or the scheduler. Confirm an invitation is single-use and expires correctly; confirm reports contain no diary text and only the first Dad/Mom addresses are selected.
...
supabase secrets set \
  RESEND_API_KEY=... \
  RESEND_ALERTS_FROM_EMAIL=onboarding@resend.dev \
...
When `RESEND_ALERTS_FROM_EMAIL=onboarding@resend.dev`, the function replaces the browser recipient list server-side with `RESEND_TEST_RECIPIENT`. For production, set `RESEND_ALERTS_FROM_EMAIL` to a verified sender display name—such as `Conquerer App <alerts@example.com>`—and leave `RESEND_TEST_RECIPIENT` only for explicit non-production test-sender use. Keep all Resend secrets in Supabase Function Secrets only; never add them to `.env`, GitHub Pages Vite variables, or browser localStorage.
