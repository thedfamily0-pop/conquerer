...
The production project (`gbjkockgfntgctchkzdk`) has the application schema through migration 015, Google-only onboarding, and the active `send-family-invitation` Edge Function. Migrations 016–020 have not yet been applied to production. On 2026-08-09, the reviewed `ai-chat`, `send-parent-alert`, `send-parent-reports`, snapshot, and weekly-research Function sources were deployed with their required Function-only secrets; their new schema-dependent paths remain inactive until migrations are applied. The live GitHub Pages build has not yet been updated with these source changes. No scheduler, real invitation, alert, or report was activated/sent during this partial rollout. The invitation Function retains its production browser origin (`https://thedfamily0-pop.github.io`) and full invitation URL (`https://thedfamily0-pop.github.io/conquerer/`) configured server-side, never as Vite variables.
