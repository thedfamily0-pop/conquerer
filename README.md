...
The production project (`gbjkockgfntgctchkzdk`) has the application schema through migration 015, Google-only onboarding, and the active `send-family-invitation` Edge Function. Migrations 016–018 and the pending report/video/recovery source changes in this repository have not been applied or deployed to production. The live GitHub Pages build was rebuilt with the hosted Supabase configuration. The Function has its production browser origin (`https://thedfamily0-pop.github.io`) and full invitation URL (`https://thedfamily0-pop.github.io/conquerer/`) configured server-side, never as Vite variables.
...
4. Set `RESEND_API_KEY` plus Function-specific sender secrets: `RESEND_WELCOME_FROM_EMAIL`, `RESEND_ALERTS_FROM_EMAIL`, `RESEND_DAILY_RECAP_FROM_EMAIL`, and `RESEND_WEEKLY_RECAP_FROM_EMAIL`. Use verified sender addresses with display names, deploy the matching Functions, and verify the sender domain.
...
8. Deploy `materialize-learning-report-snapshots` and `generate-weekly-content-research` only after migration 017 is present. Keep `LEARNING_REPORT_SNAPSHOTS_CRON_TOKEN` and `WEEKLY_CONTENT_RESEARCH_CRON_TOKEN` as separate Supabase Function Secrets. The snapshot job is intended to run after Johannesburg midnight for the previous day; the research job is intended to run around Saturday 12:00 Johannesburg, one hour before the weekly recap. Neither job sends email or publishes content. Do not activate either scheduler until a non-production test and explicit production approval.
