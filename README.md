...
### Current production rollout status and first-family procedure

The production project (`gbjkockgfntgctchkzdk`) is confirmed to have the application schema through migration 020, Google-only onboarding, and the deployed reviewed `ai-chat`, `send-parent-alert`, `send-parent-reports`, `materialize-learning-report-snapshots`, and `generate-weekly-content-research` Edge Functions with their required Function-only secrets. `send-family-invitation` remains active with its production browser origin (`https://thedfamily0-pop.github.io`) and full GitHub Pages invitation URL (`https://thedfamily0-pop.github.io/conquerer/`) configured server-side, never as Vite variables. GitHub Pages deployment of this release, scheduler activation, and the required production identity/data reset have not been independently verified. Do not infer live browser behavior, scheduled execution, delivery, or reset results from the confirmed migrations and Functions alone.

A non-sending authenticated browser smoke test from the live Pages origin previously reached the invitation Function and its RPC without creating an invitation. The Resend sending domain is verified. **A real welcome email and an end-to-end parent or child redemption have not yet been tested; do not treat provider delivery as verified until an authorised recipient receives and redeems an invitation.**

A previous public-application-data reset preserved Supabase Auth users and identities, so it does **not** meet the current fresh-first-user requirement. Before treating production as fresh, complete and independently verify the reviewed dependency-aware identity/data reset while retaining schema, migrations, RLS/RPCs, Google provider configuration, Edge Functions/secrets, verified sender configuration, GitHub Pages configuration, and reviewed `learning_calendar_weeks` reference data. Only after that verification:
...
- **Weekly research foundations**: the server-only snapshot/research jobs create parent-reviewable drafts from canonical calendar outcomes and aggregates; they do not publish content, email anyone, or run until separately deployed, tested, and scheduled
